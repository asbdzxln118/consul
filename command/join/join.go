...
type cmd struct {
	UI        cli.Ui
	flags     *flag.FlagSet
	httpFlags *flags.HTTPClientFlags
	wan       bool
}

func (c *cmd) initFlags() {
	c.flags = flag.NewFlagSet("", flag.ContinueOnError)
	c.flags.BoolVar(&c.wan, "wan", false, "Joins a server to another server in the WAN pool.")

	c.httpFlags = flags.NewHTTPClientFlags()
	c.httpFlags.AddTo(c.flags)
}
...
func (c *cmd) Help() string {
	return flags.Usage(usage, c.flags, c.httpFlags.Flags)
}
...
func (c *cmd) Run(args []string) int {
	...
	client, err := c.httpFlags.APIClient()
	if err != nil {
